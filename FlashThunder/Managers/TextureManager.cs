...
internal class TextureManager
{
    ...
    public TextureManager(ContentManager cm, string defaultTex = null)
    {
        _cache = [];
        _contentManager = cm;
    ...
    /// <returns></returns>
    public TextureManager Register(string alias, string name)
    {
        if (TryLoad(name, out Texture2D tex))
            _cache[alias] = tex;
        else
            Logger.Warn($"Texture {name} failed to register!");
    ...
    /// <param name="name"></param>
    /// <returns></returns>
    public TextureManager RegisterDefault(string name)
    ...
    /// <returns></returns>
    public TextureManager Set(string alias, Texture2D tex)
    {
        _cache[alias] = tex;
        return this;
    }
    ...
    /// <param name="tex"></param>
    /// <returns></returns>
    public TextureManager SetDefault(Texture2D tex)
        => Set(DefaultName, tex);
    ...
    /// </summary>
    public TextureManager Clear(bool clearDefault = false)
    {
        Texture2D temp = null;
    ...
    public TextureManager LoadDefinitions(string path)
    {
